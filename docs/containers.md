...
- `Format(fmt, args...)` — returns `Expected<StringUtf8, ErrorCode>` holding the formatted result.
- `AppendFormat(output, fmt, args...)` — appends the formatted result to an existing `StringUtf8` and returns `ErrorCode`.

Both report a failed allocation as `ErrorCode::OutOfMemory` rather than throwing. When `AppendFormat` fails, `output` keeps whatever was written before the string ran out of room.
...
Opal::Expected<Opal::StringUtf8, Opal::ErrorCode> msg = Opal::Format("hello {}", "world");
if (msg.HasValue())
{
    // msg.GetValue() == "hello world"
}

// Numeric formatting
auto num = Opal::Format("{} + {} = {}", 1, 2, 3);    // "1 + 2 = 3"
auto flt = Opal::Format("{:.2f}", 3.14);              // "3.14"
auto hex = Opal::Format("{:#x}", 255);                // "0xff"
auto pad = Opal::Format("{:04d}", 42);                // "0042"

// Append to an existing string
Opal::StringUtf8 str("prefix: ");
if (Opal::AppendFormat(str, "value={}", 99) == Opal::ErrorCode::Success)
{
    // str == "prefix: value=99"
}

// No args — literal passthrough
auto lit = Opal::Format("literal");                   // "literal"
```
