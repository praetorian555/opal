...
bool Opal::Paths::IsDirectory(const StringUtf8& path, AllocatorBase* allocator)
{
#if defined(OPAL_PLATFORM_WINDOWS)
...
bool Opal::Paths::IsFile(const StringUtf8& path, AllocatorBase* allocator)
{
#if defined(OPAL_PLATFORM_WINDOWS)
    StringWide path_wide(path.GetSize() * 2, L'\0', allocator);
    const ErrorCode err = Transcode(path, path_wide);
...
bool Opal::Paths::CollectDirectoryContents(const StringUtf8& path, DynamicArray<StringUtf8>& out_contents,
                                           const DirectoryContentsDesc& desc, AllocatorBase* allocator)
{
#if defined(OPAL_PLATFORM_WINDOWS)
    StringWide path_wide(path.GetSize() * 2, L'\0', allocator);
